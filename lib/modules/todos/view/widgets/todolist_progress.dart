...
class TodoListProgress extends StatelessWidget {
  final TodoList todoList;
  const TodoListProgress({Key key, this.todoList}) : super(key: key);
  ...
  double _calclulateProgress() {
    final finishedItems = todoList.todos.where((item) => item.done).length;
    if (finishedItems == 0) {
      return 0.0;
    }
    return finishedItems.toDouble() / todoList.todos.length;
  }
}
